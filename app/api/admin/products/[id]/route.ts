    ...
    const product = await db.$transaction(async tx => {
      const updated = await tx.product.update({
        ...
        data: {
          ...
          condition: condition !== undefined ? (isValidCondition(condition) ? condition : null) : existing.condition,
          cardNumber: cardNumber !== undefined ? (typeof cardNumber === 'string' && cardNumber.trim() ? cardNumber.trim() : null) : existing.cardNumber,
